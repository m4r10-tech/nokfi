...
| I | **`create-checkout` no valida `plan` — `coercePlan` cae a 'mini'** | llamada directa con `plan:"bogus"` (+email válido) crea sesión `mini` real en Stripe en vez de 400 | `config/plans.js:65` `coercePlan()` devuelve `'mini'` para cualquier plan no válido (fall-back intencionado para robustez). `routes/payments.js:106` no valida `plan` antes del primer call a Stripe; el frontend solo manda mini/pro/max (seguros). No es fallo de seguridad (sigue requiriendo pago), pero una API call directa crea sesiones huérfanas. Fix: en `create-checkout`, tras `coercePlan`, comprobar `req.body?.plan` ∈ VALID_PLANS y si no, devolver 400 `invalid_plan` (sin tocar Stripe). Cambio con su test e2e. |
