...
| K | ✅ **RESUELTO** — 1er `invoice.paid` del trial huérfano | — | En alta con trial (mini), el invoice de apertura del trial (`billing_reason='subscription_create'`, 0€) llega por webhook SIN `invoice.subscription` (Stripe lo deja undefined) → `handleStripeInvoicePaid` no podía enlazarlo a la licencia por subId → caía en `if (!license)` y se marcaba `processed:false` + warn ruidoso en cada alta. No era fallo real: la licencia ya la crea `checkout.session.completed` (y `invoice.paid` no crea licencias, solo renueva). Fix (`webhooks.js` handler `invoice.paid`): interceptar ese caso exacto (create + amount_paid 0) como **no-op reconocido** (`processed:true` + audit `INVOICE_PAID_TRIAL_OPEN_NOOP`), SIN tocar el caso genuino de "renovación/PR sin licencia mapeada" que sigue `processed:false` para reconciliación. Doble condición anti-tragacargos: un cobro real trae `subscription` seteado → se enlaza por subId → nunca cae aquí. e2e **93/93**. |
